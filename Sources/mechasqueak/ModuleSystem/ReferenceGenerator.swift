...
struct ReferenceGenerator {
    static func generate(inPath path: URL) {
        ...
        try! html.write(
            to: path.appendingPathComponent("commandref.html"), atomically: true, encoding: .utf8)
    }
}
...
extension IRCBotCommandDeclaration: HTMLRepresentable {
    var html: String {
        ...
        if self.options.count > 0 || self.arguments.count > 0 {
            html += """
                    <p style="margin-left: 30.0px;">
            ...
            for option in options {
                let optionDescription = lingo.localize(
                    "help.command.\(self.commands[0]).\(option)", locale: "en-GB")
                html += """

    ...
    var body: Content {
        if case .param(let description, _, let type, let nullability) = token {
            switch type {
                case .standard:
                    Span { description }
                        .class(parameterClasses(description: description, type: type, nullability: nullability))
...
func parameterClasses(
    description: String,
    type: CommandBody.ParameterType,
    nullability: CommandBody.ParameterNullability
) -> String {
